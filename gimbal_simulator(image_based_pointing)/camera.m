...
function out = camera(uu,P)
    ...
    for i = 1:P.num_agents,
        c(i).pn    = uu(1+NN); % MAV North position
        c(i).pe    = uu(2+NN); % MAV East position
        c(i).pd    = uu(3+NN); % MAV Down position
        c(i).phi   = uu(7+NN); % MAV roll angle
        c(i).theta = uu(8+NN); % MAV pitch angle
        c(i).psi   = uu(9+NN); % MAV yaw angle
        NN = NN + 12;
    end
    for i = 1:P.num_agents,
        c(i).az    = uu(1+NN); % gimbal azimuth angle
        c(i).el    = uu(2+NN); % gimbal elevation angle
        NN = NN + 2;
    end
    t     = uu(1+NN); % time
    ...
    out = zeros(3*P.num_agents,1);
    
    % determine pixel location and size
    for i = 1:P.num_agents,
        [c(i).eps_x, c(i).eps_y, c(i).eps_s] = cameraProjection(tn,te,td,c(i).pn,c(i).pe,c(i).pd,c(i).phi,c(i).theta,c(i).psi,c(i).az,c(i).el,P);
    end
  
    
    if t==0,  % initialize the plot
        for i = 1:P.num_agents,
            figure(3+i), clf,hold on
            % plot camera view 
            ...
            axis ij
            hold on
            blob_handle(i) = drawBlob([c(i).eps_y, c(i).eps_x], c(i).eps_s, [], 'normal');
            xlabel('px (pixels)')
            ylabel('py (pixels)')
            title('Camera View')
            grid on
        end
    else% (eps_y~=-9999 && eps_x~=-9999),
        for i = 1:P.num_agents,
            drawBlob([c(i).eps_y, c(i).eps_x], c(i).eps_s, blob_handle(i));
            hold on
        end
    end
    ...
    NN = 0;
    for i = 1:P.num_agents,
        out(NN+1:NN+3) = [c(i).eps_x; c(i).eps_y; c(i).eps_s];
        NN = NN + 3;
    end
end

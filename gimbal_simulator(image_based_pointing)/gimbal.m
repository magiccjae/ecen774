...
sizes.NumContStates  = 2*P.num_agents; % position
sizes.NumDiscStates  = 0; % velocity, last position
sizes.NumOutputs     = 2*P.num_agents; % output position and velocity
sizes.NumInputs      = 2*P.num_agents;
...
%
x0 = zeros(2*P.num_agents,1);
NN = 0;
for i = 1:P.num_agents,
    x0(NN+1:NN+2) = [P.az0; P.el0];
    NN = NN + 2;
end
...
function sys=mdlDerivatives(t,x,u,P)
    ...
    sys = zeros(2*P.num_agents,1); % initialize size of output
    
    MM = 0;
    for j = 1:P.num_agents,
        az = x(MM+1);
        el = x(MM+2);
        u_az = u(MM+1);
        u_el = u(MM+2);

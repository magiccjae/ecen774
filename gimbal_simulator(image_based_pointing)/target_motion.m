...
%  z_last = z;
 if (norm(z-z_last)>=BuildingWidth+StreetWidth),
      % randomly pick directions of motion
      temp = rand;
      if (temp < 0.25) & (z(1)<width-StreetWidth),
          vel = [P.target_velocity; 0; 0];
          z_last = z;
    elseif (temp < 0.5) & (z(2)<width-StreetWidth),
          vel = [0; P.target_velocity; 0];
          z_last = z;
      elseif (temp < 0.75) & (z(1) > StreetWidth),
          vel = [-P.target_velocity; 0; 0];
          z_last = z;
      elseif (z(2)>StreetWidth),
          vel = [0; -P.target_velocity; 0];
          z_last = z;
      else
          vel = [0; 0; 0];
